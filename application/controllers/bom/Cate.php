...
class Cate extends CI_Controller {
	...
	*/
	...
	function pageModel(){ return 'Cate_model'; }
	...
	function loadData(){
		...
		if(isset($_POST['COPY_ARR']) && $_POST['COPY_ARR'] != ''){//복사있으면
			$ARR = $this->input->post('COPY_ARR');
			foreach ($ARR as $i){
				$new_id	= $this->{$this->pageModel()}->get_new_id();	// 새 id 따기
				$this->{$this->pageModel()}->copy_cate($new_id,$i);		// 등록
				$this->{$this->pageModel()}->copy_cate_dtl($new_id,$i);	// 등록
			}
		}
    ...
    if ($page > $total_pages) 
      $page=$total_pages;
	    ...
	    foreach($query as $row) {
	        ...
	        $responce->rows[$i]['cell']=array($row->BC_ID,$row->BC_NM,$row->INS_ID,$row->INS_DT,$row->FA_CNT);
	        $i++;
	    }
