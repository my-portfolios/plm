...
class Part_write extends CI_Controller {
	/*
	...
	*/
	...
	function pageType(){ return 'Part_write'; } //seg 2
	function pageModel(){ return 'Part_write_model'; } // model
	function pageReturn($id=""){ return $id=="" ? 'Part' : 'Part_write?id='.$id; } //def,PLM_TYPE,return
	/*파일저장시*/
	function fileName(){ return 'BP_NM'; } //제목 name
	function fileCont(){ return 'BP_CONT'; } //컨텐츠 name
	function fileKeyWord(){ return '부품정보'; } //파일저장시 pdm 키워드이름
	
	function fileId(){ return 'BP_ID';}
    ...
    {   
    	...
    	if(isset($_GET['id'])){
				...
				$data['compList']	= $this->Pop_compSearch_model->getCompList('bom',$id);
	...
	public function save(){
		...
		for($i = 0; $i < count($pf_id_arr); $i++){
			$this->Common_model->insert_file_list($new_id,$pf_id_arr[$i],strtolower($this->pageReturn()),'bom_part'); // 첨부파일 등록 
		}

		//거래처 저장
		$compArr = $this->input->post("PF_COMP");
		for( $i = 0; $i < count($compArr) ; $i++ ){
			$result1 = $this->Pop_compSearch_model->insert_comp($this->uri->segment(1), $new_id , $compArr[$i]);
		}
	...
	public function upd(){
		...
		if($result_update_pdm){
			for($i = 0; $i < count($pf_id_arr); $i++){
				$this->Common_model->insert_file_list($searchData[$this->fileId()],$pf_id_arr[$i],strtolower($this->pageReturn()),'bom_part'); 
			}
		}

		//거래처 삭제 및 저장
		$bp_id = $searchData['BP_ID'];	/* 프로젝트id */
		$compArr = $this->input->post("PF_COMP");
		$result0 = $this->Pop_compSearch_model->del_comp($this->uri->segment(1), $bp_id);
		for( $i = 0; $i < count($compArr) ; $i++ ){
			$result1 = $this->Pop_compSearch_model->insert_comp($this->uri->segment(1), $bp_id , $compArr[$i]);
		}
	...
	public function delete(){
		...
		redirect($this->pageSeg().'/'.$this->pageReturn($BP_ID));
	}
