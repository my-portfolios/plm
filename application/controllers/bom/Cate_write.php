...
class Cate_write extends CI_Controller {
	/*
	...
	*/
	...
	function pageType(){ return 'Cate_write'; } //seg 2
	function pageModel(){ return 'Cate_write_model'; } // model
	function pageReturn(){ return 'Cate'; } //def,PLM_TYPE,return
	/*파일저장시*/
	function fileName(){ return 'BC_NM'; } //제목 name
	function fileCont(){ return 'BC_CONT'; } //컨텐츠 name
	function fileKeyWord(){ return '카테고리'; } //파일저장시 pdm 키워드이름
	
	function fileId(){ return 'BC_ID';}
	...
	public function save(){
		...
		//부품정보 저장
		$this->{$this->pageModel()}->del_part($searchData['BC_ID']);
	...
	public function upd(){
		...
		$this->{$this->pageModel()}->del_part($searchData['BC_ID']);
		for($i = 0; $i < count($searchData['BP_IDS']); $i++){
			$this->{$this->pageModel()}->insert_part($searchData['BC_ID'],$searchData['BP_IDS'][$i]);
		}
		
		//부품 수량테이블 정리
		$this->{$this->pageModel()}->update_part_cnt($searchData['BC_ID']);
	    ...
	    if ($page > $total_pages) 
	      $page=$total_pages;
		    ...
		    foreach($query as $row) {
		        ...
		        $responce->rows[$i]['cell']=array($row->BCD_ID,$row->BP_NMS,$row->BP_STDS,$row->BP_MTRS,$row->BP_IDS,$row->INS_DT);
		        $i++;
		    }
