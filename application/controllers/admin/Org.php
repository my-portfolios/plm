...
class Org extends CI_Controller {
	...
	*/
	...
	function pageModel(){ return 'Org_model'; }
    ...
    if ($page > $total_pages) 
      $page=$total_pages;
	    ...
	    foreach($query as $row) {
	        $responce->rows[$i]['cell']=array($row->ORG_ID,$row->ORG_NM,$row->ORG_DATA,$row->ORG_YN,$row->INS_ID,$row->INS_DT,$row->FA_CNT);
	        $responce->rows[$i]['id']=$row->ORG_ID;
	        $i++;
	    }
