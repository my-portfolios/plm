...
class Board extends CI_Controller {
	...
	*/
	...
	function pageModel(){ return 'Board_model'; }
	...
	function contsChk(){
		...
		$cnt = $this->{$this->pageModel()}->contsChk($board_id);
    ...
    if ($page > $total_pages) 
      $page=$total_pages;
	    ...
	    foreach($query as $row) {
	        $responce->rows[$i]['cell']=array($row->BOARD_ID,$row->BOARD_TITLE,$row->BOARD_AUTH,$row->BOARD_READ_AUTH,$row->BOARD_WRITE_AUTH,$row->INS_DT,$row->FA_CNT);
	        $responce->rows[$i]['id']=$row->BOARD_ID;
	        $i++;
	    }
