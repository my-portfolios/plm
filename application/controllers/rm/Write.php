...
class Write extends CI_Controller {
	...
	function pageSeg(){ return 'rm'; } //seg 1
    ...
    {      
        ...
        $this->load->model('/rm/Write_model');
    ...
    {   
		if($_GET && $_GET['id'] != 'all'){
			$pr_id = $_GET['id'];
			$data['list'] 		= $this->Write_model->getData($pr_id);
			...
			$data['fileList']	= $this->Write_model->getFileList($pr_id);
		}else{
			$data['list'] 		= null;
			$data['empList'] 	= null;
			$data['pmsList'] 	= null;
	...
	public function save(){
		...
		/* 요구사항 저장 */
		$new_pr_id	= $this->Write_model->get_new_pr_id();	// 요구사항관리 새 id 따기
		$this->Write_model->insert_rm($new_pr_id,$searchData);	// 요구사항 등록 
		for($i = 0; $i < count($pf_id_arr); $i++){
			$this->Common_model->insert_file_list($new_pr_id,$pf_id_arr[$i],$this->uri->segment(1),'normal'); // 요구사항 첨부파일 등록 
		}
		...
		for( $i = 0; $i < count($pmsArr) ; $i++ ){
			$this->Pop_pmsSearch_model->insert_pms($this->uri->segment(1), $new_pr_id , $pmsArr[$i]);
		}
		...
		for( $i = 0; $i < count($empArr) ; $i++ ){
			$this->Pop_empSearch_model->insert_emp($this->uri->segment(1), $new_pr_id , $empArr[$i], $empArrNm[$i]);
		}
	...
	public function upd(){
		...
		/* 요구사항 */
		$this->Write_model->update_rm($searchData);	 /* 요구사항 수정 */
		
		$result_update_pdm = $this->fn_update_pdm($searchData);	// 요구사항 변경내용을 pdm에도 적용
		
		if($result_update_pdm){
			for($i = 0; $i < count($pf_id_arr); $i++){
				$this->Common_model->insert_file_list($searchData['PR_ID'],$pf_id_arr[$i],$this->uri->segment(1),'normal'); // 요구사항 첨부파일 등록 
			}
		}
		
		/* 프로젝트 등록 시작 */
		$pmsArr 	= $this->input->post("PF_PMS");
		$result0 = $this->Pop_pmsSearch_model->remove_pms($this->uri->segment(1),$searchData['PR_ID']);	/* 프로젝트 삭제 */
		if($result0){
			for( $i = 0; $i < count($pmsArr) ; $i++ ){
				$this->Pop_pmsSearch_model->insert_pms($this->uri->segment(1), $searchData['PR_ID'] , $pmsArr[$i] );
			}
		}
		/* 프로젝트 등록 끝 */
		
		/* 담당자 등록 시작 */
		$result_del_emp = $this->Pop_empSearch_model->remove_emp($this->uri->segment(1),$searchData['PR_ID']);	/* 담당자 삭제 */
		if($result_del_emp){
			$empArr = $this->input->post("PF_EMP");
			$empArrNm = $this->input->post("PF_EMP_NM");
			for( $i = 0; $i < count($empArr) ; $i++ ){
				$this->Pop_empSearch_model->insert_emp($this->uri->segment(1), $searchData['PR_ID'] , $empArr[$i], $empArrNm[$i]);
			}
		}
		/* 담당자 등록 끝 */
	...
	function fn_update_pdm($searchData){
		...
		$pf_ids = $this->Write_model->get_pf_ids_pr_id($searchData['PR_ID']);
	...
	function pdm_file_upload($uploaded_files,$searchData){
		...
		for($i=0; $i<$uploaded_file_count; $i++) { 
			...
			$this->Upload_model->upload(null,$searchData['PR_TITLE'].'('.$i.')','',$searchData['PR_CONT'],null,$new_pf_id,$tmp_name,$file_name ,$_FILES['PF_FILE']['size'],$ext,'rm');	// 업로드 등록 
			
			$this->Upload_model->insert_keyword($new_pf_id,'요구사항관리');	 // 키워드 등록
