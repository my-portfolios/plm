...
class Pop_bomSearch_cate extends CI_Controller {
	 
	function __construct()
    {      
        parent::__construct();
        $this->load->model('/com/Pop_bomSearch_cate_model');
    } 
	...
	public function searchGrid(){
		...
		$query = $this->Pop_bomSearch_cate_model->getAllData('0','0',$sidx,$sord,$where); 
		$count = count($query); 
		if( $count > 0 ) {
		...
		if ($page > $total_pages) 
			$page=$total_pages;
		$query = $this->Pop_bomSearch_cate_model->getAllData($start,$limit,$sidx,$sord,$where); 
		$responce->page = $page;
		...
		foreach($query as $row) {
			...
			$responce->rows[$i]['cell']=array($row->BC_ID,$row->BC_NM,$row->INS_DT);
