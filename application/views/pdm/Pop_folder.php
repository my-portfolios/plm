...
<script>
	...
	$(function(){
		...
		$(document).on('click','.deleteFolder',function(){
			...
			var sel = $('#jstree-tree-pop-folder').jstree(true).get_selected();
			var node = $('#jstree-tree-pop-folder').jstree(true).get_node(sel);
			
			if(sel[0] == 'PLM')	return preventDefaultAction(false);
			
			var isChild = fn_chkChildren(sel[0],node);
			
			if(isChild){
				bootbox.alert({
					size:'small',
					message : '하위 폴더 및 파일이 존재합니다. \n 하위 폴더 및 파일 삭제 후 시도해주세요.',
					buttons: {
						ok: {
							label: '확인',
							className: "btn-<?php echo $this->config->item($this->uri->segment(1).'Color')?>"
	...
	/* 하위 폴더나 파일 있는지 체크 */
	function fn_chkChildren(sel,node){
		
		var isfolder = node.children.length;
		var result = true;
		...
		if(isfolder > 0){
			...
			$.ajax({
				...
				dataType: 'json',
				async : false,
				url: '<?php echo base_url();?>index.php/pdm/Pop_folder/chkChildFile',
				data: data,
	...
	/* 드래그해서 위치이동 */
	function fn_dragMove( parent_id , id ){
		...
		$.ajax({
			type: 'post',
			dataType: 'json',
			url: '<?php echo base_url();?>index.php/pdm/Pop_folder/moveFolder',
			data: data,
			success: function (data) {
	...
	/* 이름변경 */
	function fn_updateFolder( pfd_id , text ){
		...
		$.ajax({
			type: 'post',
			dataType: 'json',
			url: '<?php echo base_url();?>index.php/pdm/Pop_folder/updateFolder',
			data: data,
			success: function (data) {
				$('#jstree-tree-left').jstree(true).refresh();
			},
	...
	function fn_addFolder( parent_id , text ){
		...
		$.ajax({
			type: 'post',
			dataType: 'json',
			url: '<?php echo base_url();?>index.php/pdm/Pop_folder/addFolder',
			data: data,
			success: function (data) {
	...
	function fn_deleteFolder(delArr,sel){
		...
		$.ajax({
			type: 'post',
			dataType: 'json',
			url: '<?php echo base_url();?>index.php/pdm/Pop_folder/deleteFolder',
			data: data,
			success: function (data) {
	...
	function getFolderTree(){
		...
		jstreeObj.jstree({
			...
			if( data.old == '' ){	//create
				...
				fn_updateFolder(data.node.id,data.text);
