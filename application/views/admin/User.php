...
<script>
				...
				callback: function (result) {
					if(result == true){
						...
						for(var i = 0; i < ids.length; i++){
							...
							var value = rowObject.PE_ID;     //Obejct key값이 PR_ID value값 반환
							if(value == 'admin'){
								bootbox.alert({
									size:'small',
									message : '관리자는 삭제 할 수 없습니다.',
									buttons: {
										ok: {
											label: '확인',
											className: "btn-<?php echo $this->config->item($PageNm.'Color')?>"
	...
	function formatOpt3(cellvalue, options, rowObject){//권한
		var str ='';
	 	//str += "<span class='link' onclick=msgView('"+cellvalue+"')>"+cellvalue+"</span>";
	 	switch (cellvalue){
			 case 'user': str='사용자'; break;
			 case 'emp': str='작업자'; break;
			 case 'admin': str='관리자'; break;
		 }
		return str;
	}
	...
	$(window).load(function(){
		...
		mtype : "POST",             
		datatype: "json",            
		colNames:['아이디','이름','거래처','연락처','권한','작성자','작성일'],       
		colModel:[
			{name:'PE_ID',index:'PE_ID', width:100, align:"center",formatter:formatOpt2},
			{name:'PE_NM',index:'PE_NM', width:300, align:"left",formatter:formatOpt1},
			{name:'PC_NM',index:'PC_NM', width:150, align:"center"},
			{name:'PE_TEL',index:'PE_TEL', width:100, align:"center"},
			{name:'PE_AUTH',index:'PE_AUTH', width:100, align:"center",formatter:formatOpt3},
	...
	<div class="search_area p-20 gray_border_bottom">
		<form id="frm_search" name="frm_search" method="post" onsubmit="return false;">
			...
			<select class="form-control width_100px" id="searchField" name="searchField">
				...
				<option value="PE_TEL">연락처</option>
