...
<script>
				...
				callback: function (result) {
					if(result == true){
						...
						for(var i = 0; i < ids.length; i++){
							...
							var value = rowObject.PG_ID;     //Obejct key값이 PG_ID value값 반환
							if(value == 'admin'){
								bootbox.alert({
									size:'small',
	...
	function formatOpt(cellvalue, options, rowObject){//즐찾 셋팅
		var str ='';
	 	var gap ='-';
	...
	$(window).load(function(){
		...
		mtype : "POST",             
		datatype: "json",            
		colNames:['PG_ID','그룹명','전화번호','작성자','작성일','FA_CNT'],       
		colModel:[
            {name:'PG_ID',index:'PG_ID', width:150, hidden:true},
			{name:'PG_NM',index:'PG_NM', width:300, align:"left",formatter:formatOpt},
			{name:'PG_TEL',index:'PG_TEL', width:150, align:"center"},
			...
			{name:'FA_CNT',index:'FA_CNT', width:100, hidden:true}
		],
      rowNum:30,
      rowList:[30,100,500],
      pager: '#'+pager,
