...
<script>
				...
				callback: function (result) {
					if(result == true){
						...
						for(var i = 0; i < ids.length; i++){
							...
							var value = rowObject.ORG_ID;     //Obejct key값이 PR_ID value값 반환
							arr.push(value);
						}
						var postdata = {
							"REMOVE_ARR" :arr
	...
	function formatOpt1(cellvalue, options, rowObject){//즐찾 셋팅
		var str ='';
		/*
	 	var gap ='<button class="btn btn-default btn-xs" onclick=orgView("'+rowObject[0]+'")>미리보기 <span class="fa fa-window-restore" style="top: 0;position: relative;"></span></button>';
		*/
		var gap ='-';
		if(rowObject[6] > 0){
	...
	$(window).load(function(){
		...
		mtype : "POST",             
		datatype: "json",            
		colNames:['ORG_ID','조직도명','DATA','사용유무','작성자','작성일'],       
		colModel:[
			{name:'ORG_ID',index:'ORG_ID', width:100, align:"center", hidden:true},
			{name:'ORG_NM',index:'ORG_NM', width:300, align:"left",formatter:formatOpt1},
			{name:'ORG_DATA',index:'ORG_DATA', width:150, align:"center",hidden:true},
			{name:'ORG_YN',index:'ORG_YN', width:150, align:"center"},
...
<div id="wp_right">
	<div class="search_area p-20 gray_border_bottom">
		<form id="frm_search" name="frm_search" method="post" onsubmit="return false;">
			...
			<select class="form-control width_100px" style="width: 110px" id="searchField" name="searchField">
				<option value="ORG_NM">조직도명</option>
