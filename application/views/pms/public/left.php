...
?>
<script>
	var list = "pms_list";//그리드 아이디
	$(function(){
		$(document).on('click','.btn_pms_write',function(){
			location.href="<?php echo site_url();?>/pms/Write";
		});
		...
		$(document).on('click','.list-group-c .s_y',function(){
			...
			if( now != 'Main'){
				location.href = '<?php echo site_url();?>/pms/Main/#'+$(this).attr('m');
			}else{	
...
<div id="wp_left" class="gray_border_right">
		...
		<div class="text-center pt-20 ptb-20">
			<div class="btn-group btn-group-toggle" data-toggle="buttons">
				<button type="button" class="btn btn_pms_write btn-dark mt-10">
		...
		<ul class="list-group-c list-group" style="">
			<li class="list-group-item s_y" m="all" style="cursor:pointer;" data-toggle="tooltip" data-placement="top" title="전체프로젝트 보기">
				전체
		    <i class="pull-right fa fa-ellipsis-h text-default" style="font-size:14px"></i>
		  </li>
		  <li class="list-group-item s_y" m="m" style="cursor:pointer;" data-toggle="tooltip" data-placement="top" title="공유받은 프로젝트만 보기">
				공유받은 프로젝트
		    <i class="pull-right fa fa-share-alt text-default" style="font-size:14px"></i>
		  </li>
		  <?php if($this->session->userdata('userauth') == 'admin'){ ?>
		  <li class="list-group-item s_y" m="trash" style="cursor:pointer;" data-toggle="tooltip" data-placement="top" title="삭제된 프로젝트만 보기">
				휴지통
