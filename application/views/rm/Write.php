...
function fn_save(){
	...
	if ($('#frm_rm_write').validator('validate').has('.has-error').length === 0) {
	  ...
	  $.ajax({
		...
		if(pr_id == ""){
			$('#frm_rm_write').attr('action','<?php echo base_url();?>index.php/rm/Write/save');
		}else{
			$('#frm_rm_write').attr('action','<?php echo base_url();?>index.php/rm/Write/upd');
		}
...
<div id="wp_right">
	...
	<div class="grid_area">
	<!-- 수정 -->
	<form id="frm_rm_write" name="frm_rm_write" class="p-20" data-toggle="validator" role="form" action="" method="post" enctype="multipart/form-data" >
		<input type="hidden" id="PR_ID" name="PR_ID" value="<?php if($list){ echo $list->PR_ID; } ?>"/>
	  ...
	  <div class="form-group required">
	    ...
	    <input type="text" class="form-control" id="PR_TITLE" name="PR_TITLE" placeholder="제목을 입력해주세요." value="<?php if($list){ echo $list->PR_TITLE; } ?>" required>
	    <span class="help-block with-errors"></span>
	  </div>
	   <div class="form-group required">
	    <label for="">완료요청일<span class="req-text" title="필수입력">*</span></label>
			<div class='input-group date datetimepicker'>
				<input type="text" class="form-control" id="PR_HOPE_END_DAT" name="PR_HOPE_END_DAT" placeholder="완료요청일" value="<?php if($list){ echo $list->PR_HOPE_END_DAT; } ?>" required>
	  ...
	  <div class="form-group required">
		  <label for="">프로젝트<span class="req-text" title="필수입력">*</span></label>
			<?php 
					if( $pmsList != null ){
						$pmss = '';
						$pmsArr = '';
			...
			<div class="input-group">
			  <input id="PF_PMS_TEXT" name="PF_PMS_TEXT" type="text" data-pms="pms_input" class="form-control req-readonly" placeholder="오른쪽 검색버튼을 이용해 검색해주세요." aria-label="..." required value="<?php if($pmsList != null){ echo $pmss; } ?>">
		...
		<div class="form-group required">
		  <label for="PF_EMP">담당자<span class="req-text" title="필수입력">*</span></label>
		   <?php 
			...
			?>
			<div class="input-group">
			  <input id="PF_EMP_TEXT" name="PF_EMP_TEXT" type="text" data-emp="emp_input" required class="form-control req-readonly" placeholder="오른쪽 검색버튼을 이용해 검색해주세요." value="<?php if($empList != null){ echo $emps; } ?>">
			  <!-- PF_EMP[] -->
	  ...
	  <div class="form-group filebox">
		...
		<a onclick="if(!confirm('한번 삭제한 파일은 복구가 불가능합니다.\n그래도 삭제하시겠습니까?')) return false;" href="<?php echo site_url()?>/rm/Write/delete?fileName=<?php echo $data->FILELIST_ID ?>&id=<?php echo $this->input->get('id'); ?>">
