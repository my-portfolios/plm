...
class Pdt_model extends CI_Model{
	...
	$this->pageType();
	*/
	function pageType(){ return 'Pdt'; }
	function defListTable(){ return 'PLM_BOM_PDT'; }
	function defListId(){ return 'BPD_ID'; }
	function defListQuery(){ 	
		return '(select count(*) from PLM_FA where PLM_TYPE = "'.$this->pageType().'" and FA_ID = a.BPD_ID and FA_USER = "'.$this->session->userdata('userid').'" ) as FA_CNT,BPD_ID,BPD_CD,BPD_NM,(select PE_NM from PLM_EMP where PE_ID = a.INS_ID) as INS_ID,INS_DT';
	}
  ...
  /*삭제*/
	function p_del($ARR){
		...
		/*부품디테일 삭제*/
		$this->db->delete('PLM_BOM_PDT_PART', array('BPD_ID' => $ARR));
		/*카테고리삭제*/
		$this->db->delete('PLM_BOM_PDT_CATE', array('BPD_ID' => $ARR));
		/*수량삭제*/
		$this->db->delete('PLM_BOM_PDT_CNT', array('BPD_ID' => $ARR));
		
		/*제품 삭제시 프로그램매핑 정보도 삭제*/
