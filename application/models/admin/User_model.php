...
class User_model extends CI_Model{
	...
	$this->pageType();
	*/
	function pageType(){ return 'User'; }
	function defListTable(){ return 'PLM_EMP'; }
	function defListId(){ return 'PE_ID'; }
	function defListQuery(){ 	
		return "(select count(*) from PLM_FA where PLM_TYPE = 'user' and FA_ID = a.PE_ID and FA_USER = '".$this->session->userdata('userid')."' ) as FA_CNT,
			(
				select group_concat(b.PC_NM separator ',') from PLM_COMP b where b.PC_ID in ( select c.PC_ID from PLM_COMP_LIST c where c.PLM_TYPE = 'user' and c.PARENT_ID = a.PE_ID )
			) as PC_NM,
			PE_ID,PE_NM,PE_PWD,PE_TEL,PE_AUTH,(select PE_NM from PLM_EMP where PE_ID = a.INS_ID) as INS_ID,INS_DT";
	}
