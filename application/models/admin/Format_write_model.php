...
class Format_write_model extends CI_Model{
	
	function pageReturn(){ return 'Format'; } //def,PLM_TYPE
	function defListTable(){ return 'PLM_FORMAT'; }
	function defListId(){ return 'PF_ID'; }
	function defSeqId(){ return 'FORMAT_'; }
  ...
  function __construct(){
	...
	public function get_new_id(){
		$sql_1 = " select concat( '".$this->defSeqId()."' ,ifnull( max( cast(substr(".$this->defListId()." , 8) as unsigned) ), 0 )+1 ) as new_id from ".$this->defListTable()." ";
	...
	public function insert($new_id,$searchData){
		...
		// ' escape
		$searchData['PF_NM'] = addslashes($searchData['PF_NM']);
