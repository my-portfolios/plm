...
class Comp_write_model extends CI_Model{
	
	function pageReturn(){ return 'Comp'; } //def,PLM_TYPE
	function defListTable(){ return 'PLM_COMP'; }
	function defListId(){ return 'PC_ID'; }
	function defSeqId(){ return 'COMP_'; }
  ...
  function __construct(){
		...
		$sql = " insert into ".$this->defListTable()."
				(    PC_ID
					...
					,UPD_DT
					 ...
					 '".$new_id."'
					,'".$searchData['PC_NM']."'
					,'".$searchData['PC_NUMBER']."'
					,'".$searchData['PC_EMP_NM']."'
					,'".$searchData['PC_TEL']."'
					,'".$_SESSION['userid']."'
					,sysdate()
					,'".$_SESSION['userid']."'
	...
	public function update($searchData){
		...
		$sql = " update ".$this->defListTable()." 
				 set PC_NM		= '".$searchData['PC_NM']."'
					,PC_NUMBER	= '".$searchData['PC_NUMBER']."'
					,PC_EMP_NM	= '".$searchData['PC_EMP_NM']."'
					,PC_TEL		= '".$searchData['PC_TEL']."'
					,UPD_ID		= '".$_SESSION['userid']."'
					,UPD_DT		= sysdate()
				 where PC_ID 	= '".$searchData['PC_ID']."'
				";
